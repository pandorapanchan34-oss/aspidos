 ...
 * Core Logic (Separate Phase)
 */
'use strict';

const _C = {
  A: 0.11937, D: 28.274, G: 0.8333,
  MAINT_INTERVAL: 300000, NAGI_THRESHOLD: 0.08, MAINT_DURATION: 8000
};
...
export class Shield {
  constructor() {
    this.tick = 0;
    this.history = [];
    this.genesis = { ts: Date.now(), omega: 1.0, rho: 1.0 };
  ...
  // デモ側の core.step() 用エイリアス
  step(obs, opts = {}) { return this.process(obs, opts); }
  ...
  process(obs, opts = {}) {
    ...
    const resAvg = this._updateResonance();
    ...
    score *= (1 + (pguRatio * 0.4 + this.fatigue * 0.3 + resonanceEffect) * maintFactor);
    score = score / (1 + score * 0.6);

    if (score > 0.8 && score < 1.0) this.nearThreshold += 1;
    else this.nearThreshold = Math.max(0, this.nearThreshold - 0.5);
    ...
    if (opts.penetration) {
      this.totalPgu = this.totalPgu * 0.982 + opts.penetration * _C.D * (this.isUnderMaintenance ? 0.7 : 1.0);
    ...
    const result = {
      tick: this.tick, score: +score.toFixed(4), level,
      omega: +this.omega.toFixed(4), pguRatio: +pguRatio.toFixed(4),
      stealth: +this.nearThreshold.toFixed(1), isMaint: this.isUnderMaintenance, isCliff
    };
  ...
  _generateJitter(now, pguRatio) {
    ...
    if (this.isUnderMaintenance) return Math.sin(timeRef / 37) * 0.11 + Math.cos(timeRef / 211) * 0.06;
  ...
  _checkMaintenance(now) {
    ...
    if (Math.random() < ((now - this.lastMaintenance) / _C.MAINT_INTERVAL + this.fatigue * 0.3)) {
      this.isUnderMaintenance = true;
      this.lastMaintenance = now;
      this.vMemory *= 0.5; this.fatigue *= 0.2; this.totalPgu *= 0.9;
    }
  }
  ...
  _updateResonance() {
    ...
    const isStructured = stability < avg * 0.16;
    this.resHistory.push(isStructured ? 1 : 0);
